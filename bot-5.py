...
def get_ai_reply(user_message: str, user_name: str) -> str:
    try:
        ...
        prompt = f"{SYSTEM_PROMPT}\n\n{user_name} ne likha: {user_message}"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Hiii {user_name}! 🌸 Main Shivani hoon! Kaisa hai? 😊")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if should_reply:
        ...
        ai_reply = get_ai_reply(clean_text, user_name)
        if random.random() < 0.15:
            link = random.choice(GROUP_LINKS)
            promo = random.choice(PROMO_MESSAGES).format(link=link)
            keyboard = [[InlineKeyboardButton("🌸 Join!", url=link)]]
            await message.reply_text(ai_reply)
            await message.reply_text(promo, reply_markup=InlineKeyboardMarkup(keyboard))
        else:
            await message.reply_text(ai_reply)

    elif not is_private:
        if random.random() < 0.40:
            ai_reply = get_ai_reply(text, user_name)
            if random.random() < 0.15:
                link = random.choice(GROUP_LINKS)
                promo = random.choice(PROMO_MESSAGES).format(link=link)
                keyboard = [[InlineKeyboardButton("🌸 Join!", url=link)]]
...
async def handle_new_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result.new_chat_member.status in ["member", "administrator"]:
        ...
        keyboard = [[InlineKeyboardButton("🌸 Join Now!", url=link)]]
        try:
            await context.bot.send_message(chat_id=chat.id, text=welcome, reply_markup=InlineKeyboardMarkup(keyboard))
        except Exception as e:
